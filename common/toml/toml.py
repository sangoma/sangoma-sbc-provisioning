...
class TomlDecodeError(Exception):
    def __init__(self, linepos, msg):
        super(TomlDecodeError, self).__init__('{} (line {}, char {})'.format(msg, *linepos))
...
def loads(s, _dict=dict):
    ...
    for i in _range(len(sl)):
        ...
        if keyname:
            if sl[i] == '\n':
                raise TomlDecodeError(getslpos(i), "Key name found without value. Reached end of line.")
            if openstring:
                if sl[i] == openstrchar:
                    keyname = 2
                    openstring = False
            ...
            if sl[i] == '=':
                ...
                raise TomlDecodeError(getslpos(i), "Found invalid character in key name: '"+sl[i]+"'. Try quoting the key name.")
        ...
        if sl[i] == '\n':
            if openstring or multilinestr:
                if not multilinestr:
                    raise TomlDecodeError(getslpos(i), "Unbalanced quotes")
                if sl[i-1] == "'" or sl[i-1] == '"':
                    sl[i] = sl[i-1]
                    sl[i-3] = ' '
            elif openarr:
            ...
            if not keygroup and not arrayoftables:
                if sl[i] == '=':
                    raise TomlDecodeError(getslpos(i), "Found empty keyname. ")
                keyname = 1
    s = ''.join(sl)
    s = s.split('\n')
    multikey = None
    ...
    for nline, line in zip(_range(1, len(s)+1), s):
        pos = 0
        line = line.strip()
        if line == "":
            continue
        if multikey:
            ...
            if len(line) > 2 and line[-1] == multilinestr[0] and \
                ...
                value, vtype = _load_value(multilinestr, nline)
        ...
        if line[0] == '[':
            ...
            if line[1] == '[':
                arrayoftables = True
                line, pos = line[2:].split(']]', 1), 2
            else:
                line, pos = line[1:].split(']', 1), 1
            if line[1].strip() != "":
                raise TomlDecodeError((nline, 0), "Key group not on a line by itself.")
            groups = line[0].split('.')
            i, group_pos = 0, pos
            while i < len(groups):
                ...
                if groups[i][0] == '"' or groups[i][0] == "'":
                    ...
                    if not _groupname_re.match(groups[i]):
                        raise TomlDecodeError((nline, group_pos), "Invalid group name '"+groups[i]+"'. Try quoting it.")
                group_pos += len(groups[i]) + 1
                i += 1
            currentlevel = retval
            group_pos = pos
            for i in _range(len(groups)):
                group = groups[i]
                if group == "":
                    raise TomlDecodeError((nline, group_pos), "Can't have a keygroup with an empty name")
                try:
                    currentlevel[group]
                    if i == len(groups) - 1:
                        if group in implicitgroups:
                            ...
                            if arrayoftables:
                                raise TomlDecodeError((nline, group_pos), "An implicitly defined table can't be an array")
                        elif arrayoftables:
                            currentlevel[group].append(_dict())
                        else:
                            raise TomlDecodeError((nline, group_pos), "What? "+group+" already exists?"+str(currentlevel))
                except TypeError:
                    if i != len(groups) - 1:
            ...
            if line[-1] != "}":
                raise TomlDecodeError((nline, pos), "Line breaks are not allowed in inline objects")
            _load_inline_object(line, nline, currentlevel, multikey, multibackslash)
        elif "=" in line:
            ret = _load_line(line, nline, currentlevel, multikey, multibackslash)
            if ret is not None:
                multikey, multilinestr, multibackslash = ret
    return retval

def _load_inline_object(line, nline, currentlevel, multikey=False, multibackslash=False):
    ...
    for group in groups:
        status = _load_line(group, nline, currentlevel, multikey, multibackslash)
...
def _load_line(line, nline, currentlevel, multikey, multibackslash):
    i = 1
    pair = line.split('=', i)
    ...
    while pair[-1][0] != ' ' and pair[-1][0] != '\t' and \
        ...
        pair = line.split('=', i)
        if prev_val == pair[-1]:
            raise TomlDecodeError((nline, 0), "Invalid date or number")
    ...
    if len(pair[1]) > 2 and (pair[1][0] == '"' or pair[1][0] == "'") \
        ...
        value, vtype = _load_value(pair[1], nline)
    try:
        currentlevel[pair[0]]
        raise TomlDecodeError((nline, 0), "Duplicate keys!")
    except KeyError:
        if multikey:
...
def _load_unicode_escapes(v, nline, hexbytes, prefix):
    ...
    if v == 'true':
        ...
        for prefix in ["\\u", "\\U"]:
            if prefix in v:
                ...
                v = _load_unicode_escapes(hexbytes[0], nline, hexbytes[1:], prefix)
        v = _unescape(v)
        if v[1] == '"':
            v = v[2:-2]
        return (v[1:-1], "str")
        ...
        return (_load_array(v, nline), "array")
    elif v[0] == '{':
        inline_object = {}
        _load_inline_object(v, nline, inline_object)
...
def _load_array(a, nline):
    ...
    for i in _range(len(a)):
        a[i] = a[i].strip()
        if a[i] != '':
            nval, ntype = _load_value(a[i], nline)
            if atype:
                if ntype != atype:
